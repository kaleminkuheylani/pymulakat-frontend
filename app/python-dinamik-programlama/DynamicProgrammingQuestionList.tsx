// /python-dinamik-programlama/DynamicProgrammingQuestionList.tsx
// Client component — backend'ten dynamic-programming kategorisindeki soruları çeker.
...
const DEFAULT_GRADIENT_FROM = "#f59e0b";
const DEFAULT_GRADIENT_TO = "#6366f1";
const DEFAULT_ACCENT = "#fbbf24";

export default function DynamicProgrammingQuestionList() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/v2/questions/all`, {
      ...
      .then((data: QuestionsResponse | Question[]) => {
        ...
        const filtered = list.filter((q) => q.category === "dynamic-programming");
        setQuestions(filtered);
      })
      .catch((err) => {
        if (err.name !== "AbortError") {
          console.warn("[DynamicProgrammingQuestionList] fetch error:", err);
  ...
  if (questions.length === 0) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-12 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">Henüz dinamik programlama sorusu yok</h2>
        <p className="text-white/50 text-sm">
          Backend&apos;e eklenen dinamik programlama soruları burada listelenir.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="text-sm text-white/50 mb-4">
        <span className="text-white font-semibold">{questions.length}</span> Python dinamik programlama sorusu listeleniyor
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {questions.map((q) => {
          const lvl = (q.level || "intermediate").toLowerCase();
          return (
            <Link
              key={q.id}
            ...
            >
              ...
              <div className="relative">
                ...
                <div className="flex items-center justify-between text-xs pt-3 mt-2 border-t border-white/5">
                  <span className="text-white/40 font-mono">dinamik-programlama</span>
                  <span
