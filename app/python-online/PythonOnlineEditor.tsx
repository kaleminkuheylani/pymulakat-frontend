...
// PythonOnlineEditor — Free-style Python editor (client-only).
// Soruya bağlı değil, sadece Monaco + Pyodide runner.
...
// Başlangıç kodu — kullanıcıyı hemen bir şey denemeye teşvik eden kısa snippet
...
export default function PythonOnlineEditor() {
  ...
  const ensurePyodide = useCallback(async () => {
    ...
    pyodideReadyRef.current = (async () => {
      // self-hosted: Vercel CDN üzerinden /pyodide/v0.27.7/full/pyodide.js
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      // Top-level kodu çalıştır
      await py.runPythonAsync(code);
      // Son expression'ı eval et (eğer varsa ve print değilse)
      ...
      if (lastExpr) {
        try {
          ...
          // ignore — sadece print output'u göster
        }
      }
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        {/* SEO destek içerik — Googlebot + no-JS için */}
        <section className="mt-12 prose prose-invert max-w-none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FeatureCard
              title="Kodunu test et"
              description="Algoritmanı çalıştır, hataları gör, iteratif şekilde geliştir. Tıpkı bir Jupyter defterindeki gibi."
              icon="🧪"
            />
            <FeatureCard
              title="Python öğren"
              description="Başlangıç kodlarını çalıştır, sonuçları gör, kodu değiştir, tekrar dene. Öğrenmenin en hızlı yolu."
              icon="🎓"
            />
            <FeatureCard
              title="Snippet paylaş"
              description="Çalışan kodu mülakat hazırlığında referans olarak kullan veya arkadaşlarınla paylaş."
              icon="🔗"
            />
          </div>
          ...
          <div className="space-y-3 text-sm text-white/70">
            <FaqItem
              q="Hangi Python sürümü kullanılıyor?"
              a="Python 3.12 (CPython) WebAssembly üzerinden çalışır. Standart kütüphane ve popüler paketler (numpy, pandas, scipy, requests) kullanılabilir."
            />
            <FaqItem
              q="Kodum sunucuya gönderiliyor mu?"
              a="Hayır. Pyodide tamamen tarayıcında çalışır. Kodun veya verilerin hiçbiri sunucuya iletilmez — KVKK uyumlu ve gizli."
            />
            <FaqItem
              q="Üye olmam gerekiyor mu?"
              a="Hayır. Sayfayı aç, kod yaz, çalıştır. Hesap zorunluluğu yok."
            />
            <FaqItem
              q="Telefondan / tabletten kullanabilir miyim?"
              a="Evet. Editör responsive tasarıma sahip. Mobil cihazlarda da yazılabilir, ancak büyük ekranda daha rahat olur."
            />
          </div>

          <div className="mt-10 p-6 rounded-xl bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20">
            <h3 className="text-lg font-bold text-white mb-2">
              Python Öğrenmek mi İstiyorsun?
            </h3>
            <p className="text-sm text-white/70 mb-4">
              Sıfırdan ileri seviyeye kadar ücretsiz rehberler, mülakat soruları
              ve interaktif pratik için aşağıdaki sayfalara göz at.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link
                href="/python-egitimi"
                className="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-[#050816] font-bold text-sm"
              >
                🎓 Python Eğitimi
              </Link>
              <Link
                href="/python-kodlari"
                className="px-4 py-2 rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white font-bold text-sm"
              >
                💻 Python Kodları
              </Link>
              <Link
                href="/interviews"
                className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-sm"
              >
                📚 Mülakat Soruları
              </Link>
            </div>
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#0a0e1a] overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10 bg-white/[0.02]">
        ...
        <button onClick={onClear} className="text-[11px] text-white/40 hover:text-white/70">
          Temizle
        </button>
      </div>
